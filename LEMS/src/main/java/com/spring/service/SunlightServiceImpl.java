	...
	public Map<String, Object> getSunlightList(SearchListCommand command) throws SQLException {
		...
		if(sunlightList.size()>0) {
			...
			for(int i=0;i<sunlightList.size();i++) {
				...
				//mix, max
				flmin=(flmin>Float.parseFloat(sunlightList.get(i).getFullLight())) ? Float.parseFloat(sunlightList.get(i).getFullLight()) : flmin;
				flmax=(flmax<Float.parseFloat(sunlightList.get(i).getFullLight())) ? Float.parseFloat(sunlightList.get(i).getFullLight()) : flmax;
				eumin=(eumin>Float.parseFloat(sunlightList.get(i).getFullLight())) ? Float.parseFloat(sunlightList.get(i).getFullLight()) : eumin;
				eumax=(eumax<Float.parseFloat(sunlightList.get(i).getFullLight())) ? Float.parseFloat(sunlightList.get(i).getFullLight()) : eumax;
			}
		}	
		...
		//표준편차
		for(int i=0; i<sunlightList.size();i++) {
			fldevi+=Math.pow(((Float.parseFloat(sunlightList.get(i).getFullLight())-(flsum/sunlightList.size()))),2);
			eudevi+=Math.pow(((Float.parseFloat(sunlightList.get(i).getSunRise())-(eusum/sunlightList.size()))), 2);
		}
