	...
	<div class="card-body">
		<div class="row">
			...
			<div class="" style="position: relative; display: inline-block;">
				...
				<%@ include file="/WEB-INF/views/analysis/pagination1.jsp" %>				
			</div>
		</div>
	</div>
	 ...
	 //downloadCSV
	   function downloadCSV() {
	      ...
	      const rows = table.getElementsByTagName("tr"); // 행 저장
	       ...
	       link.setAttribute("download", "compare.csv"); // .csv 다운로드하는 링크
	 ...
	 //downloadExcel
	   function downloadExcel() {
	      ...
	      const rows = table.getElementsByTagName("tr"); // 행 저장
	       ...
	       const link = document.createElement("a");
	       link.setAttribute("href", encodedUri);
	       link.setAttribute("download", "compare.xls"); // .csv 다운로드하는 링크
	       document.body.appendChild(link);
	       link.click(); // <a> 클릭해서 다운로드 시작
	   }
...
// 	alert(typeof(labels[0]));
   ...
   var chartOptions = {
 	...
 	<c:forEach items="${daydataList}" var="daydata">
 		...
 		chartData.datasets[0].data.push(${daydata.dayEle}); //데이터 배열에 추가
 	 	chartData.datasets[1].data.push(${daydata.dayPre}); //데이터 배열에 추가
 	 </c:forEach>
</script>
</body>
</html>